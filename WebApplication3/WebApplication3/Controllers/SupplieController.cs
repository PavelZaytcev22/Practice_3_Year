...
namespace WebApplication3.Controllers
{
    ...
    {
        private readonly IService<Supplie> service;
        /// <summary>
        /// Конструктор контроллера
        /// </summary>
        /// <param name="service">Сервис контроллера для поставки</param>
        public SupplieController(IService<Supplie> service)
        {
        ...
        [HttpPost("add")]
        public async Task<int> AddAsync([FromBody] Supplie obj, CancellationToken token)
        {
            if (obj == null)
            {
        ...
        [HttpGet("getById")]
        public async Task<Supplie> GetByIdAsync(int id, CancellationToken token)
        {
            if (id <= 0)
            {
